...
$languages = array(
	'pl_PL' => array('name' => 'Polski','dir'=>'en_US'),
	'en_US' => array('name' => 'English','dir'=>'en_US')
);
...
function set_lang($lang,$charset='UTF-8'){	
	global $def_lang;
...
function select_lang($lang_key){
	if($lang_key==$_COOKIE['lang']){
			return ' selected ';			
	}
}
...
echo '<form method="POST" action="'.$_SERVER['PHP_SELF'].'" >';
	...
	echo '<select name="lang" onchange="this.form.submit()">';
		foreach($languages as $lang_k=>$lang_name){
			echo '<option value="'.$lang_k.'" '.select_lang($lang_k).'>'.$lang_name.'</option>';
		}
	echo '</select>';
